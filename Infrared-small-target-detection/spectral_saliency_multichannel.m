function S=spectral_saliency_multichannel(I,I1,imsize,multichannel_method,smap_smoothing_filter_params,cmap_smoothing_filter_params,cmap_normalization,extended_parameters,do_figures,do_channel_image_mattogrey)

  %if nargin<2, imsize=1; end % use original image size
%   if nargin<2, imsize=[NaN 64]; end
  if nargin<3, multichannel_method='quat:dct'; end
  %if nargin<4, smap_smoothing_filter_params={'anigauss',2.5}; end
  if nargin<4, smap_smoothing_filter_params={'gaussian',3,2.0}; end
  ...
  IR1=imresize(I1, imsize, 'bicubic');
  ...
  switch multichannel_method
    ...
    case {'quat:dct','quaternion:dct'}
      if isempty(extended_parameters)
        [S,DCTIR,IDCTIR]=spectral_dct_saliency_quaternion(IR,IR1);
      end
      if iscell(extended_parameters)
        [S,DCTIR,IDCTIR]=spectral_dct_saliency_quaternion(IR,IR1,extended_parameters{:});
      end
      if isstruct(extended_parameters)
        % set default values
        absexp = 2;
        ...
        if isfield(extended_parameters,'L'), L = extended_parameters.L; end
        [S,DCTIR,IDCTIR]=spectral_dct_saliency_quaternion(IR,IR1,absexp,dctaxis,L);
      end
      if ~isempty(smap_smoothing_filter_params)
        if ischar(smap_smoothing_filter_params{1}) && strcmp('anigauss',smap_smoothing_filter_params{1})
          S=anigauss(S,smap_smoothing_filter_params{2});
